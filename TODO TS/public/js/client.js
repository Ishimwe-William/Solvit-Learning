...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const btnClearCompleted = document.querySelector(".btn-link");
  ...
  function render() {
    const filtered = allTasks.filter((task) => {
      const matchesFilter = currentFilter === "all" || currentFilter === "pending" && task.status === "pending" || currentFilter === "completed" && task.status === "completed" || currentFilter === "cancelled" && task.status === "cancelled";
      const matchesSearch = !searchQuery || task.name.toLowerCase().includes(searchQuery.toLowerCase()) || task.description.toLowerCase().includes(searchQuery.toLowerCase());
